...
roberta_model = RobertaForSequenceClassification.from_pretrained("output/roberta-clariq/checkpoint-100")
local_dir = "output/t5-small-clariq/"
tokenizer = T5Tokenizer.from_pretrained(local_dir)
model = T5ForConditionalGeneration.from_pretrained(local_dir).cuda()

dataset = '../data/clariq/clariq_train.tsv'
qulac_train = pd.read_csv(dataset, delimiter='\t')
...
for iter, row in qulac_train.iterrows():
    query = normalize_text(qulac_train.at[iter, 'initial_request'])
...
qulac_train.to_csv('clariq_train.csv')
qulac_train_short = qulac_train[qulac_train['answer-len'] <= 3 ]
qulac_train_long = qulac_train[qulac_train['answer-len'] > 3 ]
qulac_train_short.to_csv('clariq_train_short.csv')
qulac_train_long.to_csv('clariq_train_long.csv')


dataset = '../data/clariq/clariq_dev.tsv'
qulac_dev = pd.read_csv(dataset, delimiter='\t')
qulac_dev.replace(['', "NaN", 'NaT'], np.nan, inplace = True)
qulac_dev.dropna(subset=['question', 'facet_desc' , 'answer'], how='any', inplace=True)
qulac_dev = qulac_dev[qulac_dev.topic_id < 201]

for iter, row in qulac_dev.iterrows():
    query = normalize_text(qulac_dev.at[iter, 'initial_request'])
...
qulac_dev.to_csv('clariq_dev.csv')
qulac_dev_short = qulac_dev[qulac_dev['answer-len'] <= 3 ]
qulac_dev_long = qulac_dev[qulac_dev['answer-len'] > 3 ]
qulac_dev_short.to_csv('clariq_dev_short.csv')
qulac_dev_long.to_csv('clariq_dev_long.csv')


dataset = '../data/clariq/clariq_test.tsv'
qulac_test = pd.read_csv(dataset, delimiter='\t')
...
for iter, row in qulac_test.iterrows():
    query = normalize_text(qulac_test.at[iter, 'initial_request'])
...
qulac_test.to_csv('clariq_test.csv')
qulac_test_short = qulac_test[qulac_test['answer-len'] <= 3 ]
qulac_test_long = qulac_test[qulac_test['answer-len'] > 3 ]
qulac_test_short.to_csv('clariq_test_short.csv')
qulac_test_long.to_csv('clariq_test_long.csv')
