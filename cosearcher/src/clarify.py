...
class Clarify:
    ...
    def step(
        ...
        answer = self.user_simulator.feedback(user_simulator_state, question)
    ...
    def run(self, epochs: int, topics: clarify_types.Topic):
        ...
        for topic in tqdm.tqdm(topics):
            ...
            for facet in topic.facets:
                ...
                for question, _ in facet.questions_answers:
                    dialogue_out = self.run_dialogue(topic, facet, question)
    ...
    def run_dialogue(self, topic: clarify_types.Topic, facet: clarify_types.Facet, question):
        dialogue_out = {}
        dialogue_out["turns"] = []
        ...
        '''
        #-----this is the original loop start -------
        step_result = self.step(state, user_sim_state, facet, question)
        #state = step_result["state"]
        dialogue_out["turns"].append(
            {
                "question": question,
        ...
        dialogue_out["metrics"] = self.ir_metric_calculator.calculate_metrics(
            self.ir_system, topic, facet, topic.query + '? ' + str(question) + '. ' + str(step_result['answer'])
            #self.ir_system, topic, facet, topic.query
        )
        return dialogue_out
