...
class LookUpGenerator(AnswerGenerator):
    def __init__(
        self,
    ...
    def generate_answer(
        ...
        answer_row = self.df.loc[(self.df['facet_desc'] == facet.desc.lower()) & (self.df['question'] == question.lower())]
        try:
            assert len(answer_row) == 1
            return answer_row[self.candidate_column_name].item()
        except:
            #print(facet.desc.lower(), answer_row)
            return ""